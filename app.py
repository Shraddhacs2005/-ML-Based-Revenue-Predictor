...
# ---------------------------
    ...
    st.header("📊 Analytics — Dataset Visualizations")
    ...
    # Clean column names (in case some are encoded already)
    st.subheader("Numeric distributions")

    # Item_Weight
    if "Item_Weight" in data_for_viz.columns:
        fig_w = px.histogram(data_for_viz, x="Item_Weight", nbins=50, title="Item Weight Distribution")
        st.plotly_chart(fig_w, use_container_width=True)
    else:
        st.info("Item_Weight not found in dataset.")

    # Item_Visibility
    if "Item_Visibility" in data_for_viz.columns:
        fig_vis = px.histogram(data_for_viz, x="Item_Visibility", nbins=50, title="Item Visibility Distribution")
        st.plotly_chart(fig_vis, use_container_width=True)

    # Item_MRP
    if "Item_MRP" in data_for_viz.columns:
        fig_mrp = px.histogram(data_for_viz, x="Item_MRP", nbins=50, title="Item MRP Distribution")
        st.plotly_chart(fig_mrp, use_container_width=True)

    # Item_Outlet_Sales (only if full dataset present)
    if "Item_Outlet_Sales" in data_for_viz.columns:
        fig_sales = px.histogram(data_for_viz, x="Item_Outlet_Sales", nbins=50, title="Item Outlet Sales Distribution")
    ...
    if "Outlet_Establishment_Year" in data_for_viz.columns:
        fig_year = px.histogram(data_for_viz, x="Outlet_Establishment_Year", title="Outlet Establishment Year Counts")
        st.plotly_chart(fig_year, use_container_width=True)

    # Item_Fat_Content
    if "Item_Fat_Content" in data_for_viz.columns:
        # if encoded ints, try to map back or just show unique counts
        if data_for_viz["Item_Fat_Content"].dtype == object:
            fig_fat = px.histogram(data_for_viz, x="Item_Fat_Content", title="Item Fat Content Counts")
        else:
            # try mapping from common encoding
            counts = data_for_viz["Item_Fat_Content"].value_counts().reset_index()
            counts.columns = ["value", "count"]
            fig_fat = px.bar(counts, x="value", y="count", title="Item Fat Content (encoded) Counts")
        st.plotly_chart(fig_fat, use_container_width=True)

    # Item_Type (may have many categories)
    if "Item_Type" in data_for_viz.columns:
        # If numeric encoded, show top categories
        try:
            top = data_for_viz["Item_Type"].value_counts().nlargest(20).reset_index()
            top.columns = ["Item_Type", "count"]
            fig_type = px.bar(top, x="count", y="Item_Type", orientation="h", title="Top Item Types")
            st.plotly_chart(fig_type, use_container_width=True)
        except Exception:
            st.info("Could not render Item_Type counts.")

    # Outlet_Size
    if "Outlet_Size" in data_for_viz.columns:
        fig_size = px.histogram(data_for_viz, x="Outlet_Size", title="Outlet Size Counts")
